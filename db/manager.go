...
func Begin(dbconn models.DB) (models.Storage, error) {
	var d db
...
func (d *db) txHandler() *sqlx.Tx {
	err := d.db.Ping()
	if err != nil {
		conn := sqlx.MustConnect("mysql", d.uname+":"+d.pwd+"tcp@("+d.ip+":3307)/HRMS")
		if err = conn.Ping(); err != nil {
...
func (d *db) Auth(uname, pwd string) (models.User, error) {
	...
	var u DbUser
	tx := d.txHandler()
	err := tx.QueryRowx("select a.uid, b.fname, b.lname, b.empstatus, b.joining, b.deleted_at from Auth a join User b on a.uid=b.uid where a.uname=? and a.pwd=password(?)", uname, pwd).StructScan(&u)
	if err != nil {
		return m, err
	}
	m.Fname = u.Fname
